...
Proof.
  intros x.
  unfold shiftn. destruct (Nat.ltb_spec x 0).
  - lia.
...
Proof.
  unfold Upn. rewrite subst_consn_shiftn. 1: reflexivity.
...
Section Confluence.
  ...
  Proof.
    ...
    destruct (Hr _ _ pr0). split.
    - exists x0. destruct s. exists x1. intuition auto.
    - now rewrite <- Heq.
  Qed.
  ...
  Proof.
    ...
    destruct IHt1.
    - pose proof (fix_app_fix mfix i (l ++ [t2])).
      forward H.
      + intros eq. destruct l; discriminate.
  ...
  Proof.
    ...
    - simpl. eapply All2_local_env_over_app.
      + constructor. 1: constructor.
        simpl. apply p.
      + revert IHX.
        generalize (List.rev Δ) (List.rev Δ'). induction 1. 1: constructor.
        * constructor. 1: auto.
          red in p0. red. red. red. red in p0.
          rewrite !app_context_assoc. cbn. apply p0.
        * constructor. 1: auto. destruct p0. unfold on_decl_over in *. simpl.
          rewrite !app_context_assoc. cbn. intuition.
    - simpl. eapply All2_local_env_over_app.
      + constructor. 1: constructor.
        simpl. unfold on_decl_over, on_decl in *. destruct p.
...
(*      | None => tProj (i, pars, narg) (rho Γ x) }; *)
    ...
    *)
    ...
    Solve All Obligations with (
      program_simpl ;
      split ; intros ; intuition auto ; discriminate
    ).
    Next Obligation.
      cbn.
      lazymatch goal with
    ...
    Proof.
      ...
      simpl in X. eapply Alli_impl. 1: eauto.
      intros.
      simpl in X0.
      rewrite - (rev_mapi (fun k x => vass (dname x) (lift0 k (dtype x)))) in X0.
    ...
    Proof.
      induction m in acc |- *; simpl. 1: reflexivity.
      now rewrite IHm.
    Qed.
    ...
    Proof.
      induction m in Γ, Δ |- *. 1: simpl; auto.
      intros.
      simpl.
    ...
    Proof.
      ...
      rewrite - fold_fix_context_over_acc'. 1: auto.
    ...
    Proof.
      ...
      induction m using rev_ind. 1: constructor.
      intros.
    ...
    Proof.
      ...
      - split; auto. eapply pred1_refl_gen. constructor; eauto with pcuic.
    ...
    Proof.
      ...
      - unfold Upn.
        rewrite !subst_consn_ge. 1: lia. 1: lia.
  ...
  Proof.
    ...
    1:{ eapply simpl_pred; simpl; rewrite ?up_Upn; sigma; try reflexivity.
        ...
        forward X0. 1: eapply Up_ctxmap; eauto.
        forward X0. 1: eapply Up_ctxmap; eauto.
        forward X0. 1: eapply pred1_subst_Up; auto.
    ...
    2:{ rewrite lift_rename rename_inst.
        ...
        destruct (Hrel i) as [_ Hi]. rewrite HΓi in Hi. simpl in Hi.
        rewrite H in Hi.
    ...
    - simpl. rewrite inst_closed0.
      + rewrite closedn_subst_instance_constr.
        eapply declared_decl_closed in H. 2: auto.
        hnf in H. rewrite H0 in H.
        toProp. auto.
    ...
    - (* Lambda congruence *)
      sigma. econstructor. 1: pcuic.
      eapply X2.
      + eapply Up_ctxmap. all: pcuic.
      + eapply Up_ctxmap. all: pcuic.
    ...
    - (* Fix congruence *)
      ...
      + unfold on_Trel in *. simpl. intuition auto.
        ...
        * rewrite -H. apply pred1_subst_Upn.
          all: rewrite ?inst_context_length ?fix_context_length //.
  ...
  Proof.
    ...
    rewrite -mkApps_nested. destruct l using rev_ind.
    - simpl. auto.
    - clear IHl0.
      intros. forward IHl.
      + move/app_eq_nil=> []. congruence.
  ...
  Proof.
    ...
    - rewrite -mkApps_nested.
      ...
      + simpl in *. move eq_mkApps: (mkApps _ l) IHl => a.
        ...
        1:{ simpl. now rewrite -mkApps_nested. }
        ...
        * eapply mkApps_eq_decompose_app in Heqapp0.
          ...
          1:{
            eapply decompose_app_rec_head. unfold decompose_app in rhofix.
            erewrite rhofix. reflexivity.
          }
          ...
          -- simpl.
             ...
             ++ destruct is_constructor eqn:isc.
                ** unfold is_constructor in *.
                   destruct (nth_error _ (rarg d)) eqn:Harg; try discriminate.
              ...
              ++ now rewrite fold_mkApps_tApp !fold_tApp !mkApps_nested map_app /=.
          -- simpl.
             now rewrite fold_mkApps_tApp !fold_tApp !mkApps_nested map_app /=.
  Qed.
  ...
  Proof.
    ...
    assert (~~ isApp t0).
    { eapply decompose_app_rec_head.
      unfold decompose_app in Heq; now erewrite Heq.
    }
    ...
    - rewrite -mkApps_nested. simpl mkApps at 1.
      destruct l.
      + destruct t0; try solve [reflexivity].
        depelim H.
      + specialize (IHl t0 H). simpl mkApps in IHl at 1.
        ...
        apply (f_equal fst) in Heq'. simpl in Heq'. subst head. rewrite -Heq.
        clear Heq f r'.
        unfold decompose_app at 1.
        simpl decompose_app_rec. rewrite decompose_app_rec_mkApps.
  ...
  Proof.
    ...
    destruct args using rev_ind; auto.
    1: apply decompose_app_rec_non_nil in eapp; try congruence.
    clear IHargs.
    assert(exists a' args', (args ++ [x]) = a' :: args') as [a' [args' Heq']].
    { clear. induction args. 1: now exists x, [].
      exists a, (args ++ [x]). now rewrite app_comm_cons.
    }
    rewrite Heq'.
    assert (~~ isApp hd).
  ...
  Proof.
    intros Happ.
    destruct l using rev_ind. 1: simpl; auto.
    move => Hl Hfix.
  ...
  Proof.
    simpl. split.
    - intros Hf Hf'; red in Hf'.
      destruct f; simpl in Hf'; try congruence.
      red in Hf; auto.
    - intros Hf. destruct f; simpl in Hf; try auto.
  Qed.
  ...
  Proof.
    ...
    - simpl. eapply All2i_impl; eauto. intros.
      simpl in X0. now rewrite Nat.add_1_r.
  Qed.
    ...
    - (* construct/cofix iota reduction *)
      ...
      + simpl.
        ...
        * rewrite rename_mkApps !map_map_compose compose_map_def /on_snd.
          ...
          rewrite -ren_shiftn. sigma.
          rewrite Upn_comp ?map_length ?fix_subst_length ?map_length //.
          2: now rewrite cofix_subst_length map_length.
          rewrite subst_consn_compose compose_ids_l.
          apply subst_consn_proper => //.
          rewrite map_cofix_subst' // !map_map_compose.
          rewrite map_cofix_subst' //.
  ...
  Proof.
    induction n; simpl; sigma. 1: reflexivity.
    rewrite -subst_compose_assoc.
  ...
  Proof.
    induction l in l' |- *.
    - intros. depelim X. destruct l'; try discriminate.
      constructor.
  ...
  Proof.
    induction l; intros H; depelim H. 1: constructor.
    constructor; auto.
  Qed.
      ...
      + forward IHX.
        { cbn. constructor.
          ...
          - red. red.
            assert (#|Δ'| = #|rho_ctx_over (rho_ctx Γ) Δ'|) by now rewrite rho_ctx_over_length.
            rewrite H0.
            rewrite - (rho_lift0 (rho_ctx Γ) (rho_ctx_over (rho_ctx Γ) Δ')).
            simpl.
  ...
  Proof.
    induction Δ; simpl; auto. destruct a as [? [?|] ?].
    all: now rewrite IHΔ.
  Qed.
  ...
  Proof.
    ...
    induction H; constructor.
    - auto.
    - red in p. red in p |- *. red. apply aux, p.
    - apply IHAll2_local_env.
    - red. split.
      + apply aux. apply p.
      + apply aux. apply p.
...
(* TODO generalize fix/cofix subst by tFix/tCofix constructor! *)
  ...
  Proof with solve_discr.
    ...
    - simpl.
      eapply (substitution0_pred1); simpl in *. all: eauto.
      rewrite app_context_nil_l in X0.
      eapply X0.

    - simpl.
      eapply (substitution0_let_pred1); simpl in *. all: eauto.
    ...
    - simpl in X0. cbn. rewrite decompose_app_mkApps; auto.
      ...
      + unfold iota_red. eapply pred_mkApps; eauto.
        ...
        * eapply All2_skipn. eapply All2_sym.
          rewrite - {1} (map_id args1).
          eapply All2_map, All2_impl; eauto. simpl. intuition.
      + congruence.
    ...
    - case le': (isFix_app (tApp M0 N0)).
      + (* Fix at head *)
        ...
        destruct args using rev_ind.
        1:{ simpl in Heq. rewrite -Heq in Hhd. discriminate. }
        ...
        * (* Shorter application does not reduce *)
          ...
          -- (* Longer application reduces *)
             ...
             { destruct y => //. split; auto.
               ...
               - rewrite nth_error_app_ge ?map_length //. 1: lia.
                 simpl.
                 case e': nth_error => [arg'|].
                 + simpl.
                   move => isC _. eapply nth_error_Some_length in e'.
                   simpl in e'. lia.
                 + discriminate.
             }
             simpl.
             ...
             { move => [mfix1 [args1 [narg [fn' [[[[Hunf Hpre] Hpre']] a]]]]] eq.
               ...
               destruct (isLambda (dbody d')) eqn:isl => //.
               move => [=] Harg Hfn'. subst fn' narg.
               eapply All2_nth_error_Some_right in Heq; eauto.
               ...
               - case: Hd'' => reddd' [Hdd' deq].
                 ...
                 rewrite rargl. rewrite (proj2 (nth_error_None _ _)).
                 + lia.
                 + discriminate.
               - repeat constructor; auto.
             }
        ...
        * (* CoFix *)
          ...
          -- (* CoFix unfolding *)
             ...
             eapply pred_cofix_case
              with (map_fix rho (rho_ctx Γ) (rho_ctx_over (rho_ctx Γ)
    ...
    - simpl; econstructor; eauto. simpl in X2.
      now rewrite !app_context_nil_l in X2.
    - simpl in *. constructor. 1: eauto.
