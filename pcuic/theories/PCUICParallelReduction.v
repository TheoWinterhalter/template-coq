...
Section ParallelReduction.
  ...
  Proof.
    ...
    intros X X0 X1 (* Xlr *) X2 X3 X4 X5 X6 X7 Xrw Xprw X8 X9 X10 X11 X12 X13 X14
    X15 X16 X17 X18 X19 X20 Γ Γ' t t0 X21.
    revert Γ Γ' t t0 X21.
    fix aux 5. intros Γ Γ' t t'.
    move aux at top.
    destruct 1; match goal with
...
(*
  ...
  *)
  ...
  Proof.
    intros A B f n y o e. subst. reflexivity.
  Qed.
  ...
  Lemma cat_subst_untyped_subslet :
    forall Γ Δ1 Δ2 σ θ,
      ...
      untyped_subslet Γ (cat_subst σ θ) (Δ1 ,,, Δ2).
  Proof.
  ...
  Definition psleft npat2 σ :=
    map (option_map (lift0 npat2)) σ.

  Lemma All_on_Some_map :
    forall A B P (f : A -> B) m,
      All (on_Some (P ∘ f)) m ->
      All (on_Some P) (map (option_map f) m).
  Proof.
    intros A B P f m h.
    apply All_map. eapply All_impl. 1: eassumption.
    intros [] ?.
  ...
  | All2_mask_true :
      forall x y m l1 l2,
        P x y ->
        All2_mask P m l1 l2 ->
        All2_mask P (true :: m) (x :: l1) (y :: l2)
  | All2_mask_false :
      forall x y m l1 l2,
        All2_mask P m l1 l2 ->
        All2_mask P (false :: m) (x :: l1) (y :: l2).

  Fixpoint maskof (σ : partial_subst) :=
    match σ with
    | Some _ :: σ => true :: maskof σ
    | None :: σ => false :: maskof σ
    | [] => []
    end.

  Lemma subs_complete_map :
    forall σ θ f,
      subs_complete (map (option_map f) σ) θ ->
      ∑ φ,
        subs_complete σ φ ×
        All2_mask eq (maskof σ) θ (map f φ).
  Proof.
    intros σ θ f h.
    induction σ as [| [] σ ih] in θ, h |- *.
    - destruct θ.
      2:{ exfalso. inversion h. }
      exists []. intuition constructor.
    - cbn in h.
      destruct θ.
      1:{ exfalso. inversion h. }
      specialize (ih θ).
      forward ih.
      { inversion h. assumption. }
      destruct ih as [φ [c e]].
      eexists (_ :: φ). cbn. split.
      + constructor. assumption.
  ...
  Proof.
    ...
    - destruct hp2 as [i hi | ind' n' ui' args' pa'].
      + (* Symmetric case of the other *)
        ...
        apply pattern_mask_length in hm2 as lm2.
        cbn in hm2. cbn in e.
        replace (i - 0) with i in e by lia.
        destruct nth_error eqn:e1.
        2:{
          apply nth_error_None in e1.
          apply untyped_subslet_length in uθ.
        ...
        2:{
          ...
          apply untyped_subslet_length in uθ.
          destruct (nth_error (subs_empty _) _) as [[]|] eqn:e3.
          - clearbody npat2. clear - e3.
            induction npat2 in t, i, e3 |- *.
            + cbn in e3. destruct i. all: discriminate.
            + cbn in e3. destruct i.
              * cbn in e3. discriminate.
              * cbn in e3. eapply IHnpat2. eassumption.
          - discriminate.
          - apply nth_error_None in e3.
            rewrite subs_empty_length in e3.
            lia.
        ...
        * apply subs_complete_app_inv in hξ as [ξ1 [ξ2 [? [hξ1 hξ2]]]].
          ...
          apply all_nth_error_All2_mask_subst.
          --- intros j e.
              eapply sub_mask_nth_error_true in e as nσ. 2: eauto.
              destruct nσ as [t [e3 e5]].
              rewrite nth_error_map.
              apply nth_error_id_mask with (i := 0) in e as e4. cbn in e4.
              rewrite nth_error_map.
              rewrite e4. cbn - [subst]. cbn.
              replace (npat2 + j - 0) with (npat2 + j) by lia.
              apply subs_complete_spec in hξ1 as [eξ1 hξ1].
              apply subs_complete_spec in hξ2 as [eξ2 hξ2].
              rewrite id_mask_length in eξ1. rewrite lm2 in eξ1.
              rewrite -> nth_error_app2 by lia.
              replace (npat2 + j - #|ξ1|) with j by lia.
              erewrite hξ2. 2: eassumption.
              rewrite e3. rewrite lift0_id.
              eexists _, _.
              intuition reflexivity.
          --- intros j e.
          ...
          --- apply untyped_subslet_length in uσ.
              lia.
          --- rewrite 2!map_length. rewrite id_mask_length. reflexivity.
        * apply subs_complete_app_inv in hξ as [ξ1 [ξ2 [? [hξ1 hξ2]]]].
          subst.
          apply all_nth_error_All2_mask_subst.
          --- intros j e.
          ...
          --- intros j e.
              ...
              { intro e4. subst.
                ...
                apply nth_error_Some_length in e3 as ln.
                rewrite linear_mask_init_length in ln.
                replace (min j npat2) with j in e by lia.
                rewrite nth_error_app2 in e.
                { rewrite list_init_length. auto. }
                rewrite list_init_length in e.
                replace (j - j) with 0 in e by lia.
                cbn in e. discriminate.
              }
              eapply subs_init_nth_error_neq in e2. 2: eauto.
              2:{
