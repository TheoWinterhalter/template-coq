...
Proof.
  ...
  - case_eq (x <? #|Γ1|) ; intro e0 ; bprop e0.
    + (* The variable is located in the mixed part.
      ...
      rename isdecl into is1, isdecl0 into is2, h1 into hx1, h2 into hx2.
      ...
      destruct (istype_type hg hx1) as [s1 ?].
      destruct (istype_type hg hx2) as [s2 ?].
      destruct (ismix_nth_sort hg hm x is1' is2') as [ss [? ?]].
      replace #|Γm| with (S x + (#|Γm| - S x))%nat by omega.
      rewrite <- !lift_llift, <- !lift_rlift.
      match goal with
      | |- _ ;;; _ |-i _ : sPack (lift0 ?x ?A) (lift0 ?x ?B) =>
        change (sPack (lift0 x A) (lift0 x B)) with (lift0 x (sPack A B))
      end.
      erewrite !safe_nth_lt.
      erewrite <- safe_nth_mix by eassumption.
      erewrite <- safe_nth_lt.
      eapply type_Rel.
      eapply @wf_llift with (Δ := []) ; [ .. | eassumption] ; eassumption.
  ...
  - destruct (IHsim1 Γ Γ1 Γ2) as [pA hpA].
    ...
    pose proof (mix_length1 hm) as ml. rewrite <- ml.
    inversion h1. inversion h2. subst.
    specialize (hpA _ _ _ hm H4 H11).
    destruct (istype_type hg hpA) as [s iA].
    inversion iA. subst.
    assert (s1 = s0).
    { cbn in H7, H8. eapply sorts_in_sort ; eassumption. }
    subst.
    assert (hm' :
              ismix Σ Γ
                    (Γ1 ,, A1)
    ...
    specialize (hpB _ _ _ hm' H5 H12).
    destruct (istype_type hg hpB) as [? iB]. inversion iB. subst.
    assert (s3 = s2).
    { cbn in H13, H14. eapply sorts_in_sort ; eassumption. }
    subst.
  ...
  - destruct (IHsim1 Γ Γ1 Γ2) as [pA hpA].
    ...
    intros Γm U1 U2 hm h1 h2.
    inversion h1. inversion h2. subst.
    specialize (hpA _ _ _ hm H3 H11).
    specialize (hpu _ _ _ hm H5 H13).
    specialize (hpv _ _ _ hm H6 H14).
    destruct (istype_type hg hpA) as [? ipA]. inversion ipA. subst.
    assert (s0 = s).
    { cbn in H8, H9. eapply sorts_in_sort ; eassumption. }
    subst.
  ...
  - destruct (IHsim1 Γ Γ1 Γ2) as [pA hpA].
    ...
    pose proof (mix_length1 hm) as ml. rewrite <- ml.
    inversion h1. inversion h2. subst.
    specialize (hpA _ _ _ hm H5 H14).
    destruct (istype_type hg hpA) as [? iA]. inversion iA. subst.
    assert (s1 = s0).
    { cbn in H8, H9. eapply sorts_in_sort ; eassumption. }
    subst.
    assert (hm' :
    ...
    { econstructor ; eassumption. }
    specialize (hpB _ _ _ hm' H6 H15).
    specialize (hpu _ _ _ hm' H7 H16).
    assert (s3 = s2).
    { destruct (istype_type hg hpB) as [? ipB]. inversion ipB. subst.
  ...
  - destruct (IHsim1 Γ Γ1 Γ2) as [pu hpu].
    ...
    pose proof (mix_length1 hm) as ml. rewrite <- ml.
    inversion h1. inversion h2. subst.
    specialize (hpu _ _ _ hm H7 H17).
    specialize (hpA _ _ _ hm H4 H14).
    ...
    { econstructor ; eassumption. }
    specialize (hpB _ _ _ hm' H6 H16).
    specialize (hpv _ _ _ hm H8 H18).
    assert (s3 = s2).
    { destruct (istype_type hg hpB) as [? ipB]. inversion ipB. subst.
      eapply sorts_in_sort ; eassumption.
    } subst.
  ...
  - destruct (IHsim1 Γ Γ1 Γ2) as [pA hpA].
    ...
    intros Γm U1 U2 hm h1 h2.
    inversion h1. inversion h2. subst.
    specialize (hpA _ _ _ hm H2 H8).
    specialize (hpu _ _ _ hm H4 H10).
    assert (s0 = s).
    { destruct (istype_type hg hpA) as [? ipA]. inversion ipA. subst.
      eapply sorts_in_sort ; eassumption.
    } subst.
  ...
  - case (declared_constructor_dec (fst Σ) ind i).
    + (* The constructor is declared. *)
      ...
      * inversion h1'. inversion h2'. subst.
        f_equal.
        all: erewrite SCommon.stype_of_constructor_eq ; reflexivity.
